...
const Header = () => {
  ...
  return <motion.header initial={{
    ...
    duration: 0.6
  }} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-card/95 backdrop-blur-md shadow-soft py-3" : "bg-transparent py-6"}`}>
      <div className="container flex items-center justify-between">
        {/* Logo */}
        <a href="#" className="flex items-center gap-3">
          <div className={`p-2 rounded-xl transition-colors ${isScrolled ? "bg-primary text-primary-foreground" : "bg-primary-foreground/20 text-primary-foreground"}`}>
            <Stethoscope className="h-6 w-6" />
          </div>
          <div>
            <p className={`font-serif font-semibold text-lg transition-colors whitespace-pre-line ${isScrolled ? "text-foreground" : "text-primary-foreground"}`}>{"MEDICINA INTERNA\nDr. Estrada Peña Jorge Uriel\nDra. Gallegos Posada Adriana"}</p>
